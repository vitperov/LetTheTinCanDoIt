...
class ResponseFilesParser:
    def __init__(self, chosen_files):
    ...
    def parse_response_and_update_files_on_disk(self, response):
        ...
        for file_path in filenames_in_response:
    ...
    def extract_filenames_from_response(self, response):
        ...
        for file_path in self.chosen_files:
    ...
    def extract_content_for_file(self, response, file_path):
        """
        ...
        if start_index == -1:
            print(f"Could not find the file marker for: {file_path}")
            return None
        ...
        if start_content_index == -1:
            print(f"Could not find the start of the content block for file: {file_path}")
            return None
        ...
        if end_content_index == -1:
            print(f"Could not find the end of the content block for file: {file_path}")
            return None
    ...
    def update_file_on_disk(self, file_path, new_content):
        """
